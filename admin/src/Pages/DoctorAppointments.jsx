...
const DoctorAppointments = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <p className="font-semibold text-gray-900 text-lg">Lịch hẹn của tôi</p>

      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
        {/* Header - Desktop */}
        <div className="hidden border-gray-200 border-b bg-gray-50 px-6 py-3 font-medium text-gray-600 text-xs uppercase tracking-wider md:grid md:grid-cols-[0.5fr_2fr_1fr_1fr_1.5fr_1fr_1fr]">
          <span>#</span>
          <span>Bệnh nhân</span>
        ...
        {appointments.map((item, index) => (
          <div
            className="flex flex-col gap-3 border-gray-100 border-b px-6 py-4 text-sm last:border-b-0 md:grid md:grid-cols-[0.5fr_2fr_1fr_1fr_1.5fr_1fr_1fr] md:items-center md:gap-0"
            key={item._id}
          >
            {/* Mobile */}
            <div className="flex flex-col gap-3 md:hidden">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  {item.payment ? (
                    <span className="rounded-lg bg-blue-100 px-2 py-0.5 font-medium text-blue-600 text-xs">
                      Online
                    </span>
                  ) : (
                    <span className="rounded-lg bg-gray-100 px-2 py-0.5 font-medium text-gray-600 text-xs">
                      Tiền mặt
                    </span>
                  )}
                </div>
                  ...
                  {item.cancelled ? (
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="cursor-pointer rounded-lg p-2 text-gray-400 transition-colors hover:bg-green-50 hover:text-green-500"
                        onClick={() => completeAppointment(item._id)}
                        type="button"
                      >
                        <img alt="" className="w-5" src={assets.tick_icon} />
                      </button>
                      <button
